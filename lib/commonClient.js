 ...
 * When preferredLanguage is set, streams whose default audio matches are ranked higher.
 * @param {Array<object>} streams - Array of stream objects.
 * @param {string|null} [preferredLanguage=null] - Optional ISO 639-2/3 language code (e.g. "eng").
 * @returns {Array<object>} Deduplicated and sorted streams.
 */
function deduplicateAndSortStreams(streams, preferredLanguage) {
    ...
    const prefLang = preferredLanguage?.toLowerCase() || null;
    
    uniqueStreams.sort((a, b) => {
        // 0. Preferred audio language (when configured)
        if (prefLang) {
            const aDefaultMatch = a.defaultAudioLanguage === prefLang ? 1 : 0;
            const bDefaultMatch = b.defaultAudioLanguage === prefLang ? 1 : 0;
            if (aDefaultMatch !== bDefaultMatch) return bDefaultMatch - aDefaultMatch;

            const aHasLang = a.audioLanguages?.includes(prefLang) ? 1 : 0;
            const bHasLang = b.audioLanguages?.includes(prefLang) ? 1 : 0;
